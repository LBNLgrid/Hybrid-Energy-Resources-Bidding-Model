...
def get_stoch_cvar_stairstep_prob_noRT_LP(esr, gen, poi, opt_data, priceDA, priceRTadd, gens, probS, PRICE_STEPS):
    """
    ...
    revenue = sum(
        probS[s] * sum(dev_cost_plus[s, t] * m.e_p[s, t] + priceDA[s, t] * m.Ph_da[s, t] for t in m.TP) for s in m.TS)
    ...
    for s in m.TS:
        ...
        for t in m.TP:
            ...
            if t > 0:
                m.cons.add(
                    m.SOC[s, t] == m.SOC[s, t - 1] + effCrg * m.Esm_ch[s, t - 1] - (1 / effDis) * m.Esm_dis[s, t - 1])
            for b in m.Tstep:
                if b < nstep - 1:
