...
def get_stoch_cvar_regular_prob(esr, gen, poi, opt_data, priceDA, priceRTadd, gens, probS):
        ...
        Returns:
        - The bid curves are regular bid curves
        ...
        esrOpCost = esr[
            "opCost"]  # $/MWh cost of operating storage, such as degradation cost. Applies', 'to MWh charged and discharged')
        ...
        for s in m.TS:
            ...
            for t in m.TP:
                ...
                if nS > 0:
                    counter = 1
                    for s1 in range(nS - 1):  # equivalent to MATLAB's 1:nS-1
                        for s2 in range(s1 + 1, nS):  # equivalent to MATLAB's s1+1:nS
                            # Non-decreasing constraints
                            for t in range(nT):
                                m.cons.add((m.Ph_da[s1, t] - m.Ph_da[s2, t]) * (priceDA[s1, t] - priceDA[s2, t]) >= 0)
                            # Consistency constraints
                            eq_idx = [t for t in range(nT) if priceDA[s1, t] == priceDA[s2, t]]
                            for t in eq_idx:
                                m.cons.add(m.Ph_da[s1, t] == m.Ph_da[s2, t])
